...
export class sideorder extends Component {
  ...
  _componentFocused = () => {
    ...
    AsyncStorage.getItem('username').then(keyValue => {
      console.log('user name', keyValue);
      this.setState({name: JSON.parse(keyValue)});
    });
    db.getAllOrders('N').then(async data => {
      console.log('getTotalOrderFromDB', JSON.stringify(data));
      this.setState({Shop_det: data});
      console.log('TotalOrder', this.state.Shop_det, 'lengtyh : ', data.length);
      ...
      var len = data.length;
      try {
        await AsyncStorage.setItem('totallenght', len.toString());
      } catch (e) {
        console.warn(e);
      }

      console.log(
        'lllllllllllllllllllllllllllllllllllllllllllll Asyncstorage kkkk',
      );
      var new_len = await AsyncStorage.getItem('totallenght');
      console.log('Asyncstorage kkkk', new_len);
      // data.map((item, i) => {
