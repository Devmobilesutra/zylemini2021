...
export default class SideSubListExtended extends Component {
  ...
  async refreshDelete(app_order_id, ItemId) {
    ...
    db.getSubBrandSearchDataForEdit(
      id,
      search,
      list1,
      JoinString,
  ...
  componentWillMount = async () => {
    ...
    db.getSubBrandSearchDataForEditNew(this.state.apporderid, '0').then(
      data => {
        this.state.list2 = [];
        ...
        for (var i = 0; i < this.state.list.length; i++) {
          for (var j = 0; j < this.state.list2.length; j++) {
            if (this.state.list[i].ItemId == this.state.list2[j].item_id) {
              this.state.list[i].quantity_one = this.state.list2[
                j
              ].quantity_one;
              this.state.list[i].quantity_two = this.state.list2[
                j
              ].quantity_two;
  ...
  render() {
    ...
    const AMOUNT = (
      <Text style={styles.amountTextStyle}>{this.state.bottles} </Text>
    );
