...
export class DataCollectionStep2 extends Component {
  ...
  SchemesArrow() {
    if (this.state.collapsed == false) {
      return (
        <View>
          <Image
            style={styles.downSublistArrowStyle}
  ...
  NextButton = () => {
    db.checkOrderIdInDb(
      ...
      console.log(len, 'lengthhhhhhhhh');
      if (len === 0) {
  ...
  SearchFilterFunction(text) {
    ...
    db.getBrandSearchDataForChangeBrandColor(
      text,
      this.state.list1,
      this.state.JoinString,
      this.state.outletId,
      this.props.datacollection.collectiontype,
    ).then(data => {
      this.state.dataSource = [];
  ...
  componentWillMount() {
    ...
    console.log(
      'collection type : ' + this.props.datacollection.collectiontype,
    );
    console.log(
      'from date to date : ' +
