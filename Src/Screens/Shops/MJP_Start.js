...
class MJP_Start extends React.Component {
    state = {
        language: 'java',
        Remarks:'',
        UI_data:[],
 ...
 var currentDateTime = year + '-' + month + '-' + date + ' ' + hours + ':' + min + ':' + sec
  ...
  onChangeEnteredBox= (text) => {
    ...
    render() {
      ...
      <TouchableOpacity  onPress={() => this.props.navigation.navigate('Meetings')} style={{marginLeft:wp('7%'), marginTop:hp('3%')}}>
      <Image
      ...
      <TouchableOpacity  onPress={() => this.props.navigation.navigate('meetings')} style={{marginLeft:('34%'), marginTop:hp('3%')}}>
      <Image
