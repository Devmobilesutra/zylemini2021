...
class MJP_one extends React.Component {
  ...
  _componentFocused() {
    ...
    db.getPlannedDates().then(data => {
      ...
      db.GetMJPMasterDetails(this.state.currentDate).then(data => {
        ...
        // this.state.TotalLen = data.length;
        this.setState({TotalLen: data.length});
        // this.props.orderTotal(this.state.TotalLen);
        var len = data.length;
        try {
          AsyncStorage.setItem('totalmeets', len.toString());
        } catch (e) {
          console.warn(e);
        }
        console.log('sadasdasdasddasdd', len);
  ...
  async PrevDate(i) {
    ...
    var month = new Date().getMonth() + 1;
    //   var nextDate = moment(month,"MM").add(-i, 'months').format('DD-MMM-YYYY');
    var new_date = moment(this.state.currentDate, 'DD-MMM-YYYY').add(
      -1,
      'days',
    );
    var day = new_date.format('DD');
    ...
    await this.setState({currentDate: NextDate});
    db.GetMJPMasterDetails(this.state.currentDate).then(data => {
      console.log('shop Card infoooooooooooooooo', data);
      this.setState({Card_data: data});
      ...
      if (dataMaster > 0) {
        db.getMeetForSyncByVibha(Meeting_Id, PlannedDate).then(data => {
          if (data.length > 0) {
            ...
            db.UpdateOrderMastersssForMeetingCancel(
              data[0].ID,
              data[0].CurrentDatetime,
              data[0].Type_sync,
              data[0].Shop_Id,
              data[0].latitude,
              data[0].longitude,
              ...
              db.getOrderMasterSyncDataFor_Meeting(Meeting_Id, 'N').then(
                dataMain => {
        ...
        db.getMeetForSyncByVibha(Meeting_Id, PlannedDate).then(data => {
          if (data.length > 0) {
            ...
            db.insertOrderMastersssForMeetingCancel(
              data[0].ID,
              data[0].CurrentDatetime,
              data[0].Type_sync,
              data[0].Shop_Id,
            ...
            if (response.data.Data.Order.hasOwnProperty('Orders')) {
              for (let i = 0; i < response.data.Data.Order.Orders.length; i++) {
                db.updateOrderMasterSyncFlag(
                  response.data.Data.Order.Orders[i].MobileGenPrimaryKey,
                );
                db.deleteMeetReportMeeting(
                  response.data.Data.Order.Orders[i].MobileGenPrimaryKey,
                );
  ...
  render() {
          ...
          <View>
            <View style={{flexDirection: 'row'}}>
              <TouchableOpacity
                ...
                style={{marginTop: hp('7.5%'), marginLeft: wp('2%')}}>
    ...
    ←
              ...
              <TouchableOpacity
                ...
                style={{marginLeft: '7%', marginTop: hp('7.1%')}}>
                <Image
            ...
            <View style={{flexDirection: 'row'}}>
              <TouchableOpacity
                onPress={() => this.PrevDate(this.state.DateNow - 1)}
                style={{marginLeft: '5%', marginTop: hp('2.5%')}}>
                <Image
              ...
              <TouchableOpacity
                onPress={() => this.NextDate(this.state.DateNow + 1)}
                style={{marginLeft: '7%', marginTop: hp('2.5%')}}>
                <Image
                    ...
                    {item.sync_flag == 'N' || item.sync_flag == null ? (
                      ...
                      item.Type_sync == 0 ? (
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() =>
                              Actions.MJP_Cancel({
                                ...
                                Meeting_Id: item.ID,
                                IsActivityDone: item.IsActivityDone,
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              Actions.MJP_two({
                                ...
                                IsActivityDone: item.IsActivityDone,
                                ActivityTitle: item.ActivityTitle,
                                currentDateTimestart: currentDateTime,
                              })
                            }
                            style={{
                              width: wp('25%'),
                        ...
                        <View
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              this.SubmitReport(item.ID, item.PlannedDate)
                            }
                            style={{
                              width: wp('25%'),
                              height: 35,
