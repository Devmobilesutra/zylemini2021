...
export class EditInlineOnCreateNewOrder extends Component {
  ...
  componentWillMount() {
    ...
    //entity_id neded
    db.getItemDiscountFromDb(entity_id, orderIds, this.props.ItemId).then(
      data => {
        var Rate = '',
          selectedDiscount = '',
          OnAmount;
  ...
  convertBTLToCS() {
    if (this.state.selectedRatePer == 'CS') {
      if (this.state.unit) {
        ...
        this.state.cs =
          parseInt(this.state.box) +
          parseInt(parseInt(this.state.unit) / parseInt(this.state.bpc));
        console.log('cs : ' + cs);
        this.state.btl = parseInt(
          parseInt(this.state.unit) % parseInt(this.state.bpc),
        );
  ...
  calculateAmount() {
    if (this.state.editRateFlagDb == 'TRUE') {
      console.log('if editRateFlagDb true bpc.......' + this.state.bpc);
      //get rate as ptr
      // if(this.state.selectedRatePer == "CS"){
      // this.state.enteredRate=this.props.ptr
      // this.setState({enteredRate:this.props.ptr})
      if (this.state.selectedRatePer == 'CS') {
        if (this.state.enteredRate) {
          console.log(
            'if entered rate is true.......' + this.state.enteredRate,
          );
          ...
          if (this.state.unit) {
            if (this.state.box) {
              quntity =
                parseInt(this.state.bpc) * parseInt(this.state.box) +
                parseInt(this.state.unit); //get all bottle qty
            } else {
              quntity = parseInt(this.state.unit); //get all bottle qty
            }
          } else if (this.state.box) {
            quntity = parseInt(this.state.bpc) * parseInt(this.state.box); //get all bottle qty
          }
      ...
      } else if (this.state.selectedRatePer == 'BTL') {
        if (this.state.enteredRate) {
          console.log(
            'if entered rate is true btl.......' + this.state.enteredRate,
          );
          this.state.ptr = this.state.enteredRate;
          var perbottle = 0,
            quntity = 0;
          ...
          console.log('adding quntity=', quntity);
          //    var amounts = quntity * perbottle;
          var amounts = quntity * this.state.enteredRate;
          amounts = Number(amounts.toFixed(2));
          // alert(amounts)
          this.state.amount = amounts;
          this.setState({amount: amounts});
  ...
  calculateAmountWithAlert() {
    ...
    if (this.state.enteredRate.length) {
      if (this.state.box || this.state.unit) {
        //set if not to 0
        if (this.state.editRateFlagDb == 'TRUE') {
          //get rate as ptr
          if (this.state.selectedRatePer == 'CS') {
  ...
  applyClickHandler(e) {
    ...
    if (this.state.box > 0 || this.state.unit > 0) {
      //set if not to 0
      ...
      if (this.state.editRateFlagDb == 'TRUE') {
        ...
        console.log('selected rate : ' + this.state.selectedRatePer);
        //get rate as ptr
      ...
      if (this.state.selectedRatePer) {
        ...
        db.checkIsOrderIdInDb(entity_id, '0', this.props.dashboard.userId).then(
          data => {
            ...
            if (this.state.isOrderIdExists.length == '0') {
              ...
              AsyncStorage.setItem(
                'app_order_id',
                JSON.stringify(app_order_id),
              );
              db.insertTABLE_TEMP_OrderMaster(
                app_order_id,
                currentDateTime,
              ...
              db.selectTempMasterDetailId(this.props.ItemId, app_order_id).then(
                dataId => {
                  if (dataId.length == 0) {
                    AsyncStorage.setItem(
                      'app_order_id',
                    ...
                    db.insertTABLE_TEMP_ORDER_DETAILS(
                      app_order_id,
                      this.props.ItemId,
                      itemName,
                      this.state.box,
                    ...
                    db.updateTABLE_TEMP_ORDER_DETAILS(
                      this.state.box,
                      this.state.unit,
                      this.state.freeBox,
                      this.state.freeUnit,
              ...
              db.getInsertedTableTempOrderMasterId(
                entity_id,
                0,
                this.props.dashboard.userId,
              ).then(data => {
                app_order_id = data[0].id;
                db.checkIsRowExistInTempMasterTable(app_order_id, '0').then(
                  datalen => {
                    ...
                    for (
                      var i = 0;
                      i < this.state.isOrderIdExistsnew.length;
                      i++
                    ) {
                      app_order_id = '';
                    ...
                    if (this.state.isOrderIdExistsnew.length) {
                      //row len
                      db.selectTempMasterDetailId(
                        this.props.ItemId,
                        app_order_id,
                      ).then(dataId => {
                        if (dataId.length == 0) {
            ...
            if (this.state.selectedDiscount) {
              db.checkDiscountAlreadyInDb(this.props.ItemId, app_order_id).then(
                data => {
                  if (data.length == '0') {
                    if (this.state.selectedDiscount == 'Rate') {
                      db.insertTABLE_DISCOUNT(
                        app_order_id,
                        'cash',
                        this.state.amount,
                        '',
      ...
      <View style={styles.container}>
        <View style={styles.oredrFreeMainContainer}>
          ...
          <View style={styles.boxColumnContainer}>
            ...
            <TextInput
              ...
              autoFocus="true"
              style={[
                styles.boxTextBoxStyle,
                {backgroundColor: this.state.backgroundbox},
              ]}
          ...
          <View style={styles.unitColumContainer}>
            <Text style={styles.unitTextStyle}>{this.state.unitLable}</Text>
            {/* //BTL */}
            <TextInput
              keyboardType="numeric"
              placeholder="0"
              style={[
                styles.unitTextBoxStyle,
                {backgroundColor: this.state.backgroundunit},
              ]}
        ...
        <View style={styles.rateContainer}>
          <View style={styles.rateColumnContainer}>
            <Text style={styles.rateTextStyle}>Rate Per</Text>
          </View>
        ...
        <View style={{flex: 0.1}}>
          <View style={styles.grossMainContainer}>
            <View style={styles.grossTextContainer}>
              <Text style={styles.grossTextStyle}>Gross Amount</Text>
            </View>
        ...
        <View style={styles.discountMainContainer}>
          <View style={styles.discountColumnContainer}>
            <Text style={styles.discountTextStyle}>Discount In</Text>
          </View>
        ...
        <TouchableOpacity
          ...
          <Text style={styles.applicableTextStyle}>Applicable Schemes :</Text>
        </TouchableOpacity>
