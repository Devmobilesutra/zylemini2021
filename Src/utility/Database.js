...
//change by vibha
UpdateOrderMastersssForMeetingCancel(id, Current_date_time, entity_type, entity_id, latitude, longitude, total_amount, from_date, to_date, collection_type, user_id, remark, selected_flag, sync_flag, check_date, DefaultDistributorId, ExpectedDeliveryDate,ActivityStatus){
...
//change by vibha
insertOrderMastersssForMeetingCancel(id, Current_date_time, entity_type, entity_id, latitude, longitude, total_amount, from_date, to_date, collection_type, user_id, remark, selected_flag, sync_flag, check_date, DefaultDistributorId, ExpectedDeliveryDate,ActivityStatus) {
  //   this.initDB().then((db) => {
    return new Promise((resolve) => {
  db1.transaction((tx) => {
    tx.executeSql(
      `insert into OrderMaster(id,Current_date_time ,entity_type,entity_id ,latitude ,longitude ,total_amount ,from_date ,to_date ,collection_type ,user_id ,remark,selected_flag ,sync_flag ,check_date,DefaultDistributorId,ExpectedDeliveryDate,ActivityStatus)
                                                                  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        id, Current_date_time, entity_type, entity_id, latitude, longitude, total_amount, from_date, to_date, collection_type, user_id, remark, selected_flag, sync_flag, check_date, DefaultDistributorId, ExpectedDeliveryDate,ActivityStatus
...
//change by vibha
  ...
  getCustomerShopName(entity_id,orderid) {
    var query
    if(orderid){
      query = 'select * from Pcustomer,OrderMaster where CustomerId = "' + entity_id + '" and entity_id = "' + entity_id + '" and OrderMaster.id = "' + orderid + '"';
    }else{
     query = 'select * from Pcustomer,OrderMaster where CustomerId = "' + entity_id + '" and entity_id = "' + entity_id + '"'
     } //console.log("checkIsOrderIdInDb=", query)
    return new Promise((resolve) => {
      //  this.initDB().then((db) => {    
      db1.transaction((tx) => {
...
//change by vibha
  ...
  getOrderMasterSyncData(sync_flag) {
    ...
    return new Promise((resolve) => {
      db1.transaction((tx) => {
        tx.executeSql('select id as ID,entity_type as EntityType,entity_id as EntityID ,latitude as Latitude ,longitude as Longitude ,total_amount as TotalAmount ,from_date as FromDate ,to_date as ToDate ,collection_type as CollectionType ,user_id as UserID ,remark as Remark,Current_date_time as CurrentDatetime,DefaultDistributorId as DefaultDistributorId,ExpectedDeliveryDate as ExpectedDeliveryDate,ActivityStatus as ActivityStatus from OrderMaster where sync_flag = ?', [sync_flag], (tx, results) => {
          var OrderMaster = []
          for (let i = 0; i < results.rows.length; i++) {
            OrderMaster.push(results.rows.item(i));
          }
