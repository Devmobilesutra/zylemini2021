...
// ,"ExpectedDeliveryDate": null
  ...
  deleteAllTable() {
    this.initDB().then((db) => {
      db.transaction((tx) => { tx.executeSql('DELETE FROM table_user', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM user', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM sqlite_sequence', []).then(([tx, results]) => { }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM PDistributor', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM Sales', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM Target', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM Pcustomer', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM DistributorContacts', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM DistributorDataStatus', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM SalesYTD', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM UserCustomerItem', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM ReportControlMaster', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM Report', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM Settings', []).then(([tx, results]) => { }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM AreaParentList', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM PItem', []).then(([tx, results]) => { }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM SIPREPORT', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM AdvanceReports', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM TABLE_TEMP_OrderMaster', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM TABLE_TEMP_ImagesDetails ', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM OrderMaster ', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM OrderDetails ', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM uses_log', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM ImagesDetails', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM TABLE_TEMP_ORDER_DETAILS ', []).then(([tx, results]) => {  }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM newpartyoutlet ', []).then(([tx, results]) => { }); })
      db.transaction((tx) => { tx.executeSql('DELETE FROM newpartyImageoutlet ', []).then(([tx, results]) => {  }); })
      ...
      db.transaction((tx) => { tx.executeSql('DELETE FROM SubGroupMaster  ', []).then(([tx, results]) => {  }); })
    }).catch((err) => {
...
//if for mjp
  ...
  //change by vibha
  insertOrderDetailsedit(order_id, item_id, item_Name, quantity_one, quantity_two, small_Unit
    , large_Unit, rate, Amount, selected_flag, sync_flag) {
    // alert("detail inserted to main")
  ...
  updateDetailMainedit(quantity_one, quantity_two, small_Unit, large_Unit, rate, Amount, order_id, item_id) {
    return new Promise((resolve) => {
      // this.initDB().then((db) => {
      db1.transaction((tx) => {
        tx.executeSql('UPDATE OrderDetails SET quantity_one = ?, quantity_two = ?, small_Unit = ?, large_Unit = ?,rate = ?, Amount = ? WHERE order_id = ? and item_id = ? ', [quantity_one, quantity_two, small_Unit, large_Unit, rate, Amount, order_id, item_id]).then(([tx, results]) => {
  ...
  //change by vibha
  insertTABLE_TEMP_ORDER_DETAILS(order_id, item_id, item_Name, quantity_one, quantity_two,large_Unit, small_Unit, from_date, to_date, rate, bpc, Amount, selected_flag, bottleQty) {
...
insertMJPMaster(MJPMaster_data){
  if (MJPMaster_data.length) {
    //this.initDB().then((db) => {
      db1.transaction((tx) => { tx.executeSql('DELETE FROM MJPMaster  ', [])
      .then(([tx, results]) => {
        db1.transaction((tx) => {
...
insertMJPMasterDetails(MJPMasterDetails_data){
  if (MJPMasterDetails_data.length) {
    //this.initDB().then((db) => {
      db1.transaction((tx) => { tx.executeSql('DELETE FROM MJPMasterDetails  ', [])
      .then(([tx, results]) => { 
        db1.transaction((tx) => {
...
insertSubGroupMaster(SubGroupMaster_data){
  if (SubGroupMaster_data.length) {
    //this.initDB().then((db) => {
     ...
     // db.transaction((tx) => { tx.executeSql('DELETE FROM MJPMasterDetails  ', []).then(([tx, results]) => {  }); })
    //  db.transaction((tx) => { tx.executeSql('DELETE FROM SchemeDetails  ', []).then(([tx, results]) => {  }); })
      db1.transaction((tx) => { tx.executeSql('DELETE FROM SubGroupMaster  ', [])
      .then(([tx, results]) => { 
        db1.transaction((tx) => {
...
insertSchemeDetails_data(SubSchemeDetails_data){
  if (SubSchemeDetails_data.length) {
    //this.initDB().then((db) => {
      db1.transaction((tx) => { tx.executeSql('DELETE FROM SchemeDetails  ', [])
      .then(([tx, results]) => { 
        db1.transaction((tx) => {
...
getMeetForSyncByVibha(Meeting_Id,plannedDate) {
 ...
 // var query = "select ID as ID,Type_sync as EntityType,Shop_Id as EntityID,PlannedDate as FromDate,PlannedDate as ToDate,Remarks || ' ' || location as Remark,collection_type as CollectionType,latitude as Latitude,longitude as Longitude,TotalAmount as TotalAmount,UserID as UserID,CurrentDatetime as CurrentDatetime,DefaultDistributorId as DefaultDistributorId,ExpectedDeliveryDate as ExpectedDeliveryDate from MeetReport where IsActivityDone ='0' and Meeting_Id='" + Meeting_Id + "'"
  var query = "select * from MeetReport where PlannedDate ='" + plannedDate + "' and Meeting_Id='" + Meeting_Id + "'"
  return new Promise((resolve) => {
    db1.transaction((tx) => {
      tx.executeSql(query, [], (tx, results) => {
        var MeetReport = []
...
getRemarksForCancelMeeting(id){
  ...
  return new Promise((resolve) => {
    const products = [];
    //   var query = "select distinct CustomerId as id ,Party as Party,Outlet_Info as Outlet_Info from Pcustomer where CustomerId='" + id + "' union select  distinct OrderID as id ,OutletName as Party ,OutletAddress as Outlet_Info from newpartyoutlet where OrderID ='" + id + "' order by Party asc"
    var query = "select remark as Remarks,ActivityStatus from OrderMaster where ActivityStatus != '' and id ='"+id+"'"
    console.log("getShopCardInfo",query);
    //  this.initDB().then((db) => {
    db1.transaction((tx) => {
      tx.executeSql(query, [], (tx, results) => {
...
//change by vibha
UpdateOrderMastersssForMeetingCancel(id, Current_date_time, entity_type, entity_id, latitude, longitude, total_amount, from_date, to_date, collection_type, user_id, remark, selected_flag, sync_flag, check_date, DefaultDistributorId, ExpectedDeliveryDate,ActivityStatus,ActivityStart,ActivityEnd){
    ...
    return new Promise((resolve) => {
      db1.transaction((tx) => {
        tx.executeSql(
          ...
          [Current_date_time,latitude, longitude, remark, sync_flag, ActivityStatus,collection_type,ActivityStart,ActivityEnd, DefaultDistributorId],
          (tx, results) => {
            resolve(results);
          },
          (err) => { console.error("error=", err); }
...
//change by vibha
insertOrderMastersssForMeetingCancel(id, Current_date_time, entity_type, entity_id, latitude, longitude, total_amount, from_date, to_date, collection_type, user_id, remark, selected_flag, sync_flag, check_date, DefaultDistributorId, ExpectedDeliveryDate,ActivityStatus,ActivityStart,ActivityEnd) {
  //   this.initDB().then((db) => {
    return new Promise((resolve) => {
  db1.transaction((tx) => {
    tx.executeSql(
...
{
  var query = "select * from MeetReport where Meeting_Id ='" + meetingId + "'"
  return new Promise((resolve) => {
    db1.transaction((tx) => {
      tx.executeSql(query, [], (tx, results) => {
        var MeetReport = []
...
//change by vibha
  ...
  //change by vibha
  getSubBrandSearchDataForEdit(BrandId, searchkey, list1, joinString, orderid,outletid,collectiontype) {
    ...
    return new Promise((resolve) => {
      ...
      db1.transaction((tx) => {
        tx.executeSql(query, [], (tx, results) => {
         ...
         tempfilter.map((item, i) => {
          console.log('item id : '+ item.ItemId);
          var query1 = 'SELECT TABLE_TEMP_ORDER_DETAILS.id,TABLE_TEMP_ORDER_DETAILS.item_id, TABLE_TEMP_ORDER_DETAILS.rate,TABLE_TEMP_ORDER_DETAILS.Amount, TABLE_TEMP_ORDER_DETAILS.quantity_one,TABLE_TEMP_ORDER_DETAILS.quantity_two,TABLE_TEMP_ORDER_DETAILS.bottleQty FROM TABLE_TEMP_ORDER_DETAILS ,TABLE_TEMP_OrderMaster where TABLE_TEMP_ORDER_DETAILS.order_id = TABLE_TEMP_OrderMaster.id and TABLE_TEMP_OrderMaster.entity_id="' + outletid + '" and TABLE_TEMP_OrderMaster.collection_type="' + collectiontype + '" and TABLE_TEMP_ORDER_DETAILS.order_id ="'+orderid +'" and TABLE_TEMP_ORDER_DETAILS.item_id ="'+item.ItemId +'"'
              ...
              if(results1.rows.length == 0){
               // this.getOrderDataForAddEdit1OnLoad(order_id, itemid)
                    ...
                    db1.transaction((tx) => {
                      ...
                      //  var tempfilter = [];
                        if(results2.rows.length !=0){
                          for (let j = 0; j < results2.rows.length; j++) {
                           // tempfilter[i].push(results2.rows.item(j));
                           tempfilter[i].quantity_one = results2.rows.item(j).quantity_one
                           tempfilter[i].quantity_two = results2.rows.item(j).quantity_two
                           tempfilter[i].largeunit = results2.rows.item(j).large_Unit
                           tempfilter[i].smallunit = results2.rows.item(j).small_Unit
                           tempfilter[i].rate = results2.rows.item(j).rate
                           tempfilter[i].amount = results2.rows.item(j).Amount
                ...
                for (let k = 0; k < results1.rows.length; k++) {
                 ...
                 tempfilter[i].quantity_one = results1.rows.item(k).quantity_one
                 tempfilter[i].quantity_two = results1.rows.item(k).quantity_two
                 tempfilter[i].largeunit = results1.rows.item(k).large_Unit
                 tempfilter[i].smallunit = results1.rows.item(k).small_Unit
 ...
 // SELECT OrderDetails.order_id,OrderDetails.item_id, OrderDetails.quantity_one,  OrderDetails.rate,  OrderDetails.Amount,  OrderDetails.quantity_two, OrderDetails.large_Unit,  OrderDetails.small_Unit,  OrderMaster.from_date,  OrderMaster.to_date  FROM OrderDetails,OrderMaster where order_id="512021184814" and OrderDetails.order_id = OrderMaster.id
  ...
  var query = ' SELECT TABLE_TEMP_ORDER_DETAILS.id,TABLE_TEMP_ORDER_DETAILS.item_id, TABLE_TEMP_ORDER_DETAILS.rate,TABLE_TEMP_ORDER_DETAILS.Amount, TABLE_TEMP_ORDER_DETAILS.quantity_one,TABLE_TEMP_ORDER_DETAILS.quantity_two,TABLE_TEMP_ORDER_DETAILS.bottleQty FROM TABLE_TEMP_ORDER_DETAILS ,TABLE_TEMP_OrderMaster where TABLE_TEMP_ORDER_DETAILS.order_id = TABLE_TEMP_OrderMaster.id and TABLE_TEMP_OrderMaster.entity_id="' + outlet_id + '" and TABLE_TEMP_OrderMaster.collection_type="' + collection_type + '" and TABLE_TEMP_ORDER_DETAILS.order_id ="'+order_id +'" and TABLE_TEMP_ORDER_DETAILS.item_id ="'+itemid +'"' 
        ...
        tx.executeSql(query, [], (tx, results) => {
          ...
          if(results.rows.length == 0){
           // this.getOrderDataForAddEdit1OnLoad(order_id, itemid)
            ...
            var query = 'SELECT OrderDetails.order_id,OrderDetails.item_id, OrderDetails.quantity_one,  OrderDetails.rate,  OrderDetails.Amount,  OrderDetails.quantity_two, OrderDetails.large_Unit,  OrderDetails.small_Unit,  OrderMaster.from_date,  OrderMaster.to_date  FROM OrderDetails,OrderMaster where order_id="' + order_id + '" and OrderDetails.item_id="'+itemid+'" and OrderDetails.order_id = OrderMaster.id ';
...
console.log(' in getOrderDataForAddEditToShow')
 ...
 // SELECT OrderDetails.order_id,OrderDetails.item_id, OrderDetails.quantity_one,  OrderDetails.rate,  OrderDetails.Amount,  OrderDetails.quantity_two, OrderDetails.large_Unit,  OrderDetails.small_Unit,  OrderMaster.from_date,  OrderMaster.to_date  FROM OrderDetails,OrderMaster where order_id="512021184814" and OrderDetails.order_id = OrderMaster.id
  ...
  //  var query = 'SELECT OrderDetails.ID,OrderDetails.ItemID, OrderDetails.LargeUnit,  OrderDetails.Rate,  OrderDetails.Amount,  OrderDetails.SmallUnit, OrderDetails.FreeLargeUnit,  OrderDetails.FreeSmallUnit,  OrderMaster.FromDate,  OrderMaster.ToDate  FROM OrderDetails,OrderMaster where OrderID="' + order_id + '" and OrderDetails.OrderID = OrderMaster.ID ';  
//  var query = ' SELECT TABLE_TEMP_ORDER_DETAILS.id,TABLE_TEMP_ORDER_DETAILS.item_id, TABLE_TEMP_ORDER_DETAILS.rate,TABLE_TEMP_ORDER_DETAILS.Amount, TABLE_TEMP_ORDER_DETAILS.quantity_one,TABLE_TEMP_ORDER_DETAILS.quantity_two,TABLE_TEMP_ORDER_DETAILS.bottleQty FROM TABLE_TEMP_ORDER_DETAILS ,TABLE_TEMP_OrderMaster where TABLE_TEMP_ORDER_DETAILS.order_id = TABLE_TEMP_OrderMaster.id and TABLE_TEMP_OrderMaster.entity_id="' + outlet_id + '" and TABLE_TEMP_OrderMaster.collection_type="' + collection_type + '" and TABLE_TEMP_ORDER_DETAILS.id ="'+order_id +'" and TABLE_TEMP_ORDER_DETAILS.item_id ="'+itemid +'"' 
  var query = 'SELECT OrderDetails.order_id,OrderDetails.item_id, OrderDetails.quantity_one,  OrderDetails.rate,  OrderDetails.Amount,  OrderDetails.quantity_two, OrderDetails.large_Unit,  OrderDetails.small_Unit,  OrderMaster.from_date,  OrderMaster.to_date  FROM OrderDetails,OrderMaster where order_id="' + order_id + '"and item_id="'+itemid+'" and OrderDetails.order_id = OrderMaster.id ';
  console.log("query==", query)
      // this.initDB().then((db) => {
      // if(isOpen == 'false'){
      //   this.initDB()
  ...
  getTotalamountOfOrder(orderid) {
    return new Promise((resolve) => {
      const products = [];
      var query = 'select sum(Amount) as TotalAmount from OrderDetails where OrderDetails.order_id = "' + orderid + '"'
...
//change by vibha
  getTotalOrdersOfOrder(orderid) {
    return new Promise((resolve) => {
      const products = [];
      var query = 'select count(*) as TotalCount from OrderDetails where OrderDetails.order_id = "' + orderid + '"'
  ...
  //change by vibha
  checkIsOrderIdInDbTempOrderMasterEdit(entity_id, collection_type, user_id,orderid) {

    const products = [];
    var query = 'SELECT id,Current_date_time,entity_type,entity_id,latitude,longitude,total_amount,collection_type,user_id FROM TABLE_TEMP_OrderMaster where entity_id = "' + entity_id + '" and collection_type  = "' + collection_type + '" and user_id = "' + user_id + '" and id = "' + orderid + '"';
...
//change by vibha
  ...
  //change by vibha
  updateTABLE_TEMP_ORDER_DETAILS(qty_1,qty_2,large_Unit, small_Unit,from_date,to_date,amt, rate, bottleQty, order_id, item_id) {
  //  alert("order updated")
    return new Promise((resolve) => {
      // this.initDB().then((db) => {
      // if(isOpen == 'false'){
      ...
      db1.transaction((tx) => {
        ...
        tx.executeSql('UPDATE TABLE_TEMP_ORDER_DETAILS SET quantity_one = ?, quantity_two = ?, small_Unit = ?, large_Unit = ?, from_date = ?, to_date = ?, Amount = ?, rate = ? ,bottleQty = ? WHERE order_id = ? and item_id = ? ', 
        [qty_1, qty_2, small_Unit, large_Unit, from_date, to_date, amt, rate, bottleQty, order_id, item_id]).then(([tx, results]) => {
          resolve(results.length);
          alert("order updated")
        });
      }).then((result) => {
  ...
  getOrdersFromDbIfPresentedit(entity_id, collection_type, item_id,orderid) {
    ...
    var query = 'SELECT TABLE_TEMP_ORDER_DETAILS.id,TABLE_TEMP_ORDER_DETAILS.item_id,TABLE_TEMP_ORDER_DETAILS.rate,TABLE_TEMP_ORDER_DETAILS.bpc,TABLE_TEMP_ORDER_DETAILS.Amount,TABLE_TEMP_ORDER_DETAILS.quantity_one,TABLE_TEMP_ORDER_DETAILS.quantity_two,TABLE_TEMP_ORDER_DETAILS.small_Unit,TABLE_TEMP_ORDER_DETAILS.large_Unit,TABLE_TEMP_ORDER_DETAILS.from_date, TABLE_TEMP_ORDER_DETAILS.to_date,TABLE_TEMP_ORDER_DETAILS.selected_flag,TABLE_TEMP_ORDER_DETAILS.order_id FROM TABLE_TEMP_ORDER_DETAILS , TABLE_TEMP_OrderMaster where TABLE_TEMP_ORDER_DETAILS.order_id = TABLE_TEMP_OrderMaster.id and TABLE_TEMP_OrderMaster.entity_id="' + entity_id + '" and TABLE_TEMP_OrderMaster.collection_type="' + collection_type + '" and TABLE_TEMP_ORDER_DETAILS.item_id = "' + item_id + '" and TABLE_TEMP_ORDER_DETAILS.order_id  = "' + orderid + '"'
  ...
  deleteRowItemFromOrdrDetail(order_id, item_id) {
    return new Promise((resolve) => {
      ...
      var query = 'DELETE FROM OrderDetails where item_id = "' + item_id + '" and order_id="' + order_id + '" '
      //console.log("query==",query)
      db1.transaction((tx) => {
        tx.executeSql(query, [], (tx, results) => {
          if(results.rows.length > 0){
  ...
  deleteDiscount(order_id, item_id) {
    return new Promise((resolve) => {
      ...
      db1.transaction((tx) => {
        ...
        tx.executeSql('DELETE FROM TABLE_DISCOUNT WHERE OrderedItemID = ? and OrderID = ?', [item_id,order_id]).then(([tx, results]) => {
  ...
  discardDiscount(order_id) {
    return new Promise((resolve) => {
      ...
      db1.transaction((tx) => {
        ...
        tx.executeSql('DELETE FROM TABLE_DISCOUNT WHERE OrderID = ?', [order_id]).then(([tx, results]) => {
  ...
  discardFromOrdrDetail(order_id) {
    return new Promise((resolve) => {
      ...
      var query = 'DELETE FROM OrderDetails where  order_id="' + order_id + '" '
      //console.log("query==",query)
      db1.transaction((tx) => {
        tx.executeSql(query, [], (tx, results) => {
      ...
      db1.transaction((tx) => {
        tx.executeSql(query, [], (tx, results) => {
          ...
          for (let i = 0; i < results.rows.length; i++) {
           ...
           var query1 = 'SELECT id,order_id,item_id,item_Name,quantity_one,quantity_two,small_Unit,large_Unit,rate,Amount FROM OrderDetails WHERE  order_id ="' + order_id + '" '
            ...
            tx.executeSql(query1, [], (tx, results1) => {
              ...
              for (var i = 0; i < orderDatatemp.length; i++) {
                for (var j = 0; j < orderDataDetail.length; j++) {
                  console.log('in for ')
                  if (orderDatatemp[i].item_id == orderDataDetail[j].item_id && orderDatatemp[i].order_id == orderDataDetail[j].order_id) {
...
//change by vibha
  checkOrderIdInDbeditNext(collection_type,order_Id) {
    return new Promise((resolve) => {
      ...
      var query = 'SELECT * FROM TABLE_TEMP_ORDER_DETAILS  where TABLE_TEMP_ORDER_DETAILS.order_id="' + order_Id + '"'
  ...
  deleteTempOrderDetailsvibha(itemid, collection_type,orderid) {
    return new Promise((resolve) => {
      var query = 'delete from TABLE_TEMP_ORDER_DETAILS WHERE item_id = "' + itemid + '" and order_id ="' + orderid + '" '
  ...
  insertRecordInOrderMasterForShopCheckIn(id, Current_date_time, entity_type, entity_id, latitude, longitude,
    ...
    remark, check_date, DefaultDistributorId, ExpectedDeliveryDate,Activitystatus,activityStart,activityend) {
    return new Promise((resolve) => {
      db1.transaction((tx) => {
   ...
   //change by vibha
   updateCheckoutOrderMaster(collectiontype,shopId,checkDate,checkoutDatetime,latitude,longitude) {
    return new Promise((resolve) => {
    db1.transaction((tx) => {
      tx.executeSql(
        'UPDATE OrderMaster SET ActivityEnd=?,latitude=?,longitude=? where entity_id = "' + shopId + '" and collection_type  = "' + collectiontype + '" and check_date = "' + checkDate + '"',
    ...
    return new Promise((resolve) => {
      ...
      db1.transaction((tx) => {
        tx.executeSql(query, [], (tx, results) => {
          ...
          //alert(JSON.stringify(checkorder))
          console.log("sarassssssssssssssssssssssssssssssssss= shop :",checkorder)
          resolve(checkorder);
        });
      })
        .then((result) => {
...
//change by vibha
      ...
      db1.transaction((tx) => {
        ...
        tx.executeSql(
          ...
          [
           ...
           String(app_order_id), String(BeatID), String(outletNAme), String(contact), String(OwnerName), String(address), String(remark), String(userLatitude), String(userLongitude), String(Is_Sync), String(curentDatetime), String(ShopType), String(RegistrationNo), String(ShopId), String(ContactPerson), String(ShopArea)
          ],
          (tx, results) => {
  ...
  getOrderMasterSyncData(sync_flag) {
    ...
    return new Promise((resolve) => {
      db1.transaction((tx) => {
        tx.executeSql('select id as ID,entity_type as EntityType,entity_id as EntityID ,latitude as Latitude ,longitude as Longitude ,total_amount as TotalAmount ,from_date as FromDate ,to_date as ToDate ,collection_type as CollectionType ,user_id as UserID ,remark as Remark,Current_date_time as CurrentDatetime,DefaultDistributorId as DefaultDistributorId,ExpectedDeliveryDate as ExpectedDeliveryDate,ActivityStatus as ActivityStatus,ActivityStart,ActivityEnd  from OrderMaster where sync_flag = ?', [sync_flag], (tx, results) => {
          var OrderMaster = []
          for (let i = 0; i < results.rows.length; i++) {
            OrderMaster.push(results.rows.item(i));
          }
...
getnewPartyTargetId(){
  ...
  updateORDER_DETAILSEdit(qty_1, qty_2, small_Unit, large_Unit, from_date, to_date, amt, rate, bottleQty, order_id, item_id) {
    alert("Data updated")
    return new Promise((resolve) => {
      // this.initDB().then((db) => {
      // if(isOpen == 'false'){
